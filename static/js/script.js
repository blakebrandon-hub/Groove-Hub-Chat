...
socket.on('sync_video', (data) => {
    ...
    videoKeys.forEach(key => {
        let video = video_queue[key];
        let li = document.createElement("li");
        li.innerText = video.title; 
        element.appendChild(li);
    });

    // Ensure the player starts playing the first video if none is playing
...
function onPlayerReady(event) {
    // Check if there is at least one video in the queue
    if (Object.keys(video_queue).length > 0) {
        player.loadVideoById(video_queue[currentVideoIndex].video_id, currentTime);
    } else {
        console.log("Video queue is empty, waiting for videos to be added.");
    }
}
...
// This function is triggered when there is an error playing the video
    var errorCode = event.data;
  
    // Error codes 101 and 150 indicate video embedding issues
    if (errorCode === 101 || errorCode === 150) {
        console.log("This video is unavailable to be embedded.");

        setTimeout(function() {
            socket.emit('song_error');
        }, 5000);
    }
}
...
socket.on('update_queue', function(queue) {
    ...
    if (video_queue.length >= 1) {
        loadVideoById(video_queue[0].video_id);
    }
});
...
socket.on('resolve_error', function(queue) {
    video_queue = queue;
    let element = document.getElementById('queueList');
    element.innerHTML = ""; // Clear the existing list
    ...
    videoKeys.forEach(key => {
        ...
        if (video_queue.length >= 1) {
            loadVideoById(video_queue[currentVideoIndex].video_id);
        }
    });
});
